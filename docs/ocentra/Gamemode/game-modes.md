...
**Note:** Unity uses its own multiplayer system. Our web backend uses Solana for both multiplayer and single player matches. GameMode properties provide game configuration defaults regardless of match type.
...
**Important:** 
- Unity uses its own multiplayer system (Unity Netcode, Unity Services Lobbies).
- Our web backend uses **Solana for both multiplayer and single player matches**.
- GameMode properties provide game configuration defaults (turnDuration, maxRounds, etc.) that are used regardless of match type.
- See `docs/Multiplayer and Solana/README.md` for Solana multiplayer architecture (separate system from GameMode).
