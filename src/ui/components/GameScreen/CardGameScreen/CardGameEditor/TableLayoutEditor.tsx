...
const TableLayoutEditor: React.FC<TableLayoutEditorProps> = ({ containerRef }) => {
  ...
  // Only register dev shortcuts in dev mode
  useEffect(() => {
  ...
  const handleSavePreset = useCallback(async () => {
    ...
    // Only attempt save in dev mode
    if (!import.meta.env.DEV) {
      console.warn('[TableLayoutEditor] Save is only available in development mode');
  ...
  // Early return if not in dev mode (safety check)
  if (!import.meta.env.DEV) {
    return null;
  }
