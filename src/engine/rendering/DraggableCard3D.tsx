...
export function DraggableCard3D({
  card,
  position = [0, 0, 0],
  rotation = [0, 0, 0],
  isHovered = false,
  ...
  const handlePointerDown = (event: React.PointerEvent) => {
    if (!isDraggable || !groupRef.current) return
  ...
  useFrame((state) => {
    ...
    if (groupRef.current) {
      const targetY = (isHovered || hovered || isDragging) ? position[1] + 0.1 : position[1]
    ...
    >
      ...
      >
        <meshStandardMaterial
          color={isSelected ? '#fbbf24' : (isDragging ? '#3b82f6' : '#ffffff')}
          roughness={0.1}
          metalness={0.1}
        />
      </Box>
